...
class TransactionHeaderRepository extends PsRepository {
  TransactionHeaderRepository(
      {@required PsApiService psApiService, @required TransactionHeaderDao transactionHeaderDao}) {
  ...
  Future<dynamic> getAllTransactionList(StreamController<PsResource<List<TransactionHeader>>> transactionListStream,
      bool isConnectedToInternet, String loginUserId, int limit, int offset, PsStatus status,
    ...
    transactionListStream.sink.add(await _transactionHeaderDao.getAll(finder: finder, status: status));
    ...
    if (isConnectedToInternet) {
      ...
      if (_resource.status == PsStatus.SUCCESS) {
        ...
        transactionListStream.sink.add(await _transactionHeaderDao.getAll(finder: finder));
      } else {
        transactionListStream.sink.add(await _transactionHeaderDao.getAll(finder: finder));
      }
    }
  ...
  Future<PsResource<TransactionHeader>> postTransactionSubmit(
      Map<dynamic, dynamic> jsonMap, bool isConnectedToInternet, PsStatus status,
      {bool isLoadFromServer = true}) async {
    final String jsonMapData = jsonMap.toString();
   //print(jsonMapData);

    final PsResource<TransactionHeader> _resource = await _psApiService.postTransactionSubmit(jsonMap);
    if (_resource.status == PsStatus.SUCCESS) {
      return _resource;
    } else {
      final Completer<PsResource<TransactionHeader>> completer = Completer<PsResource<TransactionHeader>>();
