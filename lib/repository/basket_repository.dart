...
class BasketRepository extends PsRepository {
  ...
  Future<dynamic> getAllBasketList(StreamController<PsResource<List<Basket>>> basketListStream, PsStatus status) async {
    final dynamic subscription = _basketDao.getAllWithSubscription(
        ...
        onDataUpdated: (List<Basket> productList) {
          if (status != null && status != PsStatus.NOACTION) {
           //print(status);
            basketListStream.sink.add(PsResource<List<Basket>>(status, '', productList));
          } else {
           //print('No Action');
          }
        });
  ...
  Future<dynamic> addAllBasket(
      StreamController<PsResource<List<Basket>>> basketListStream, PsStatus status, Basket product) async {
    await _basketDao.insert(primaryKey, product);
    basketListStream.sink.add(await _basketDao.getAll(status: status));
  }

  Future<dynamic> updateBasket(StreamController<PsResource<List<Basket>>> basketListStream, Basket product) async {
    await _basketDao.update(product);
    basketListStream.sink.add(await _basketDao.getAll(status: PsStatus.SUCCESS));
  }

  Future<dynamic> deleteBasketByProduct(
      StreamController<PsResource<List<Basket>>> basketListStream, Basket product) async {
    await _basketDao.delete(product);
    basketListStream.sink.add(await _basketDao.getAll(status: PsStatus.SUCCESS));
  }

  Future<dynamic> deleteWholeBasketList(StreamController<PsResource<List<Basket>>> basketListStream) async {
